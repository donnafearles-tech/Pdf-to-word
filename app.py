...
# Inicializar estado de resultado
if "resultado" not in st.session_state:
    st.session_state.resultado = None  # almacenará {'exito': bool, 'ruta': str, 'mensaje': str}
...
def ocr_en_imagenes(doc, idioma="spa+eng"):
    """Extrae texto de imágenes usando OCR (opcional)."""
    if not PYTESSERACT_AVAILABLE:
        st.warning("OCR no disponible: pytesseract no está instalado.")
        return []
    textos_ocr = []
    try:
...
def preservar_tablas(doc_original):
    """Extrae tablas del documento original."""
    tablas_data = []
    try:
...
def extraer_muestra_representativa(doc_temp):
    """Extrae muestra limpia de texto, saltando portada/índice."""
    parrafos_validos = []
...
def detectar_idioma_muestra(texto_muestra, groq_api_key):
    """Detecta idioma usando langdetect (si disponible) o Groq."""
...
def llamar_groq_con_reintento(texto_lote, groq_api_key, idioma_origen="inglés", max_reintentos=None):
    """Llama a Groq con backoff exponencial."""
        ...
        try:
            ...
            if es_rate_limit and intento < max_reintentos - 1:
                ...
                if es_rate_limit:
                    ...
                    st.warning(f"⚠️ Error en API Groq: {str(e)[:100]}")
                return None
    return None
...
def obtener_estilos_validos(doc):
    """Obtiene lista de estilos válidos."""
    try:
...
def procesar_docx_multilingue(docx_path, docx_salida_path, groq_api_key, idioma_origen="inglés", tamano_lote=None):
    """Procesa el DOCX: limpia, traduce y guarda."""
    ...
    if img_comprimidas > 0:
        st.info(f"🗜️ Se comprimieron {img_comprimidas} imágenes.")
    ...
    for i in range(0, total, tamano_lote):
        ...
        if lote_completado:
            texto_estado.text(f"⏩ Lote {lote_numero}/{total_lotes} recuperado de memoria...")
        else:
...
# Mostrar advertencias sobre dependencias opcionales
if not PYTESSERACT_AVAILABLE:
...
with st.sidebar:
    ...
    with col2:
        if st.button("💾 Resetear Defaults"):
            ...
            # Limpiar también el resultado en sesión
...
# --- Lógica de subida y procesamiento con persistencia de resultado ---
archivo_subido = st.file_uploader("Selecciona el libro o documento en formato PDF", type=["pdf"])

# Si se sube un nuevo archivo, limpiar resultado anterior
if archivo_subido is not None:
    # Si hay un resultado previo, lo borramos porque vamos a procesar otro
        ...
        try:
            ...
            if exito_adobe:
                ...
                with st.spinner("Fase 2/3: Traduciendo a español y limpiando..."):
                    procesar_docx_multilingue(temp_docx, temp_docx, GROQ_API_KEY, idioma_origen=idioma_detectado, tamano_lote=CONFIG["tamano_lote"])
                
                st.success("🎉 ¡Procesamiento completado!")
                exito_total = True
                
                # Mover a resultados y guardar en session_state
            ...
            if not exito_total and os.path.exists(temp_docx):
                ...
                pass  # ya se movió
    st.rerun()  # Forzar rerun para mostrar el resultado guardado
