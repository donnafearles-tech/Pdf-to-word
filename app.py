...
# =====================================================================
# IMPORTACIÓN OPCIONAL DE LANGDETECT (DETECCIÓN DE IDIOMA)
# =====================================================================
try:
...
DEFAULT_CONFIG = {
    "tamano_lote": 8,                 # Ahora puede ir hasta 30 (slider)
    "max_reintentos": 5,
    "min_width_cm": 1.5,
    "min_height_cm": 1.5,
    "inter_lote_sleep": 3.0,
    "save_frequency": 2,
...
# =====================================================================
# 1. MOTOR DE CONVERSIÓN (ADOBE SDK V4) - SOLO PARA PDF
# =====================================================================
...
# =====================================================================
# 3. DETECCIÓN DE IDIOMA AUTOMÁTICA ROBUSTA (SIN BOTONES)
# =====================================================================
def extraer_muestra_representativa(doc_temp):
    """Extrae una muestra larga y limpia del documento, saltando portada/índice."""
    parrafos_validos = []
    for p in doc_temp.paragraphs:
        texto = p.text.strip()
        if len(texto) > 40 and not texto.isdigit():
            texto_limpio = pre_limpiar_ocr(texto)
            if len(texto_limpio) > 40:
                parrafos_validos.append(texto_limpio)
    if not parrafos_validos:
        return ""
    # Intentar encontrar el inicio del contenido real
    inicio = 0
    for i, txt in enumerate(parrafos_validos):
        if re.search(r'\b(LIVRO|CAPÍTULO|CHAPTER|BOOK|PARTE|INTRODUÇÃO|ÍNDICE)\b', txt, re.IGNORECASE):
            inicio = max(0, i - 2)  # retroceder un poco para no cortar
            break
    if inicio == 0:
        # Si no encuentra, salta los primeros 15 párrafos (portada)
        inicio = min(15, len(parrafos_validos) // 3)
    # Tomar una muestra larga: 30 párrafos o 3000 caracteres
    muestra = " ".join(parrafos_validos[inicio:inicio+30])
    # Si la muestra es muy corta, tomar más
    if len(muestra) < 800 and len(parrafos_validos) > inicio+40:
        muestra = " ".join(parrafos_validos[inicio:inicio+50])
    return muestra[:3000]  # limitar a 3000 caracteres

def detectar_idioma_con_groq(texto_muestra, groq_api_key):
    """Usa Groq con un prompt específico para distinguir bien portugués/español."""
    try:
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "Eres un lingüista forense. El texto puede tener ruido OCR. "
                        "Tu tarea es identificar el IDIOMA ORIGINAL del texto (el idioma en que fue escrito, no el de la traducción). "
                        "Responde ÚNICAMENTE con el nombre del idioma en español, en minúsculas, una sola palabra. "
                        "Si tienes dudas entre español y portugués, fíjate en palabras características: "
                        "'do', 'da', 'ão', 'ções', 'dade' (portugués) vs 'del', 'de la', 'ción', 'dad' (español). "
                        "Ejemplos válidos: portugués, español, inglés, francés, alemán, italiano."
                    )
                },
                {"role": "user", "content": texto_muestra[:2000]}
            ],
            temperature=0,
            max_tokens=10
        )
        ...
        # Verificar si el modelo devolvió algo como "portugués" o "español"
        if idioma in ["portugués", "portugues", "português"]:
            return "portugués"
        elif idioma in ["español", "espanol", "castellano"]:
            return "español"
        elif idioma in ["inglés", "ingles"]:
            return "inglés"
        elif idioma in ["francés", "frances"]:
            return "francés"
        elif idioma in ["alemán", "aleman"]:
            return "alemán"
        elif idioma in ["italiano"]:
            return "italiano"
        else:
            return "desconocido"
    except Exception as e:
...
def detectar_idioma_automatico(texto_muestra, groq_api_key):
    """Intenta langdetect, si falla o es ambiguo (español/portugués) usa Groq con reglas."""
    if not texto_muestra or len(texto_muestra) < 100:
        return "desconocido"
    
    # Primero limpiar bien
    texto_limpio = pre_limpiar_ocr(texto_muestra)
    
    # Intentar con langdetect
    if LANGDETECT_AVAILABLE:
        try:
            codigo = detect(texto_limpio[:1500])
            mapa = {'pt': 'portugués', 'es': 'español', 'en': 'inglés',
                    'fr': 'francés', 'de': 'alemán', 'it': 'italiano'}
            idioma = mapa.get(codigo, None)
            # Si langdetect dice español, hacer una verificación extra para no confundir con portugués
            if codigo == 'es':
                # Buscar marcadores fuertes de portugués
                texto_lower = texto_limpio.lower()
                portugues_markers = ['ão', 'ções', 'dade', 'do ', 'da ', 'dos ', 'das ', 'para o', 'com o']
                espanol_markers = ['ción', 'dad', 'del ', 'de la ', 'para el', 'con el']
                pt_score = sum(texto_lower.count(m) for m in portugues_markers)
                es_score = sum(texto_lower.count(m) for m in espanol_markers)
                if pt_score > es_score * 1.2:
                    idioma = 'portugués'
                elif es_score > pt_score * 1.2:
                    idioma = 'español'
                else:
                    # Si están igualados, usar Groq para desempatar
                    idioma = detectar_idioma_con_groq(texto_muestra, groq_api_key)
            return idioma
        except Exception as e:
            st.warning(f"langdetect falló: {str(e)}")
...
def llamar_groq_con_reintento(texto_lote, groq_api_key, idioma_origen="inglés", max_reintentos=None):
    ...
    if idioma_origen.lower() == "español":
        # Si el origen es español, en lugar de mantenerlo, traducimos ligeramente para mejorar redacción
        instruccion_traduccion = "El texto está en español. Mejora la redacción, corrige errores ortográficos y elimina basura OCR. Mantén el significado original."
    else:
        instruccion_traduccion = f"Traduce del {idioma_origen} al ESPAÑOL de forma precisa y natural."
                ...
                messages=[
                    {
                        ...
                        "content": (
                            ...
                            "Recibirás múltiples bloques de texto separados por exactamente: '<<BLOCK_SEPARATOR>>'\n"
                            "Para CADA bloque:\n"
                            "1. Traduce o mejora según la instrucción.\n"
                            ...
                            "Mantén el mismo número de bloques. Sin introducciones ni explicaciones."
                        )
                    },
                    {"role": "user", "content": texto_lote}
                ],
                temperature=0.1,
                max_tokens=4000  # Aumentado porque el modelo puede manejar más
            )
...
st.markdown("Sube **PDF o DOCX** escaneados → se convierten a Word (si es PDF), se traducen al español y se limpia el ruido OCR.")
...
with st.sidebar:
    ...
    CONFIG["tamano_lote"] = st.slider("Tamaño de lote (párrafos por lote)", 3, 30, CONFIG["tamano_lote"])
...
# Subida de archivos (PDF o DOCX)
archivo_subido = st.file_uploader("Selecciona el libro o documento en formato PDF o Word (DOCX)", type=["pdf", "docx"])
...
if archivo_subido and st.button("🚀 Comenzar Procesamiento Editorial", type="primary"):
    ...
    try:
        ...
        if archivo_subido.type == "application/pdf":
            ...
            # Convertir PDF a Word con Adobe
            ...
            # Es DOCX, copiar directamente
            with open(temp_docx, "wb") as f:
                f.write(archivo_subido.getbuffer())
            st.info("📄 Archivo Word subido directamente. No se requiere conversión.")
        ...
        with st.spinner("🔍 Detectando idioma del documento..."):
            ...
            if texto_muestra and len(texto_muestra) > 150:
                idioma_detectado = detectar_idioma_automatico(texto_muestra, GROQ_API_KEY)
                st.info(f"🌍 Idioma detectado: **{idioma_detectado.capitalize()}**")
            else:
                idioma_detectado = "inglés"
                st.warning("⚠️ No se pudo detectar idioma con suficiente texto. Asumiendo inglés.")
