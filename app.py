...
def flatten_elements(elements: Any, page_num: int = None, bounds: Dict = None) -> List[Dict]:
    """
    Recorre recursivamente cualquier estructura (lista, dict, etc.) y extrae una lista plana
    de elementos con texto, página y bounds.
    """
    flat = []
    ...
    if isinstance(elements, list):
        for elem in elements:
            flat.extend(flatten_elements(elem, page_num, bounds))
        return flat
    
    # Si no es un diccionario, ignorar
    if not isinstance(elements, dict):
        return flat
    
    # Ahora elements es un dict
    current_page = elements.get('Page', page_num)
    current_bounds = elements.get('Bounds', bounds)
    
    # Extraer texto si existe
    text = elements.get('Text', '').strip()
    if text and current_page is not None:
        # Asegurar que current_bounds sea un diccionario
    ...
    # Procesar hijos (puede ser lista o dict)
    children = elements.get('Elements', [])
    if children:
...
def group_elements_by_line(elements: List[Dict], page_num: int) -> List[List[Dict]]:
    ...
    # Filtrar solo elementos que sean diccionarios y tengan Page y Text
    page_elements = []
    for e in elements:
        if not isinstance(e, dict):
            continue
        if e.get('Page') == page_num and e.get('Text', '').strip():
            # Asegurar que 'Bounds' sea un diccionario
            if 'Bounds' not in e or not isinstance(e['Bounds'], dict):
                e['Bounds'] = {}
            page_elements.append(e)
    
    if not page_elements:
        return []
    
    # Ordenar por coordenada Y (top)
    ...
    for elem in page_elements:
        ...
        if current_y is None or abs(y - current_y) <= y_tolerance:
            ...
            # Ordenar la línea completa por X (left)
...
def convert_json_to_word(json_bytes: bytes, config: CleanupConfig) -> bytes:
    ...
    # Aplanar completamente la estructura
    ...
    pages = sorted(set(e.get('Page', 1) for e in flat_elements if isinstance(e, dict)))
    ...
    for page_num in pages:
        ...
        for line in lines:
            line_text = ' '.join(elem.get('Text', '') for elem in line if isinstance(elem, dict)).strip()
            if line_text:
                # Detección simple de títulos: línea corta y sin punto final
