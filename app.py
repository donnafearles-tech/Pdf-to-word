...
try:
    ...
    # NOTA: No usamos DetectorFactory.seed() porque causa TypeError en algunas versiones
...
def ocr_en_imagenes(doc, idioma="spa+eng"):
    ...
    if not PYTESSERACT_AVAILABLE:
        st.warning("OCR no disponible: pytesseract no está instalado o Tesseract no se encuentra en el sistema.")
        return []
...
# =====================================================================
# 3. DETECCIÓN DE IDIOMA (VERSIÓN MEJORADA CON LANGDETECT + GROQ)
# =====================================================================
def extraer_muestra_representativa(doc_temp):
    """
    Extrae una muestra limpia de texto del DOCX, saltándose la portada, índice y ruido inicial.
    Busca el primer párrafo que parezca inicio del contenido real (p.ej., que contenga 'LIVRO' o capítulo).
    """
    # Recopilar todos los párrafos con más de 30 caracteres
    parrafos_validos = []
    ...
    # Intentar encontrar el inicio real (primer capítulo)
    inicio = 0
    ...
    # Si no se encuentra, empezar después de los primeros 10 párrafos (para saltar portada)
    if inicio == 0:
        inicio = min(10, len(parrafos_validos) // 2)
    
    # Tomar hasta 20 párrafos a partir de ahí
    muestra = " ".join(parrafos_validos[inicio:inicio+20])
    return muestra

def detectar_idioma_con_groq(texto_muestra, groq_api_key):
    """Fallback usando Groq cuando langdetect no está disponible o falla."""
    try:
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "Eres un lingüista experto. El texto puede contener ruido de escáner (caracteres extraños) y mezcla de idiomas. "
                        "Tu única tarea es identificar el IDIOMA ORIGINAL del texto (ignorando títulos, pies de página y números). "
                        "Responde EXCLUSIVAMENTE con el nombre del idioma en español, todo minúsculas, una sola palabra. "
                        "Ejemplos válidos: portugués, español, inglés, francés, alemán, italiano. "
                        "Si el texto es portugués, responde 'portugués'. Si es español, responde 'español'. "
...
def detectar_idioma_muestra(texto_muestra, groq_api_key):
    """
    Detecta el idioma de una muestra de texto.
    Prioriza langdetect si está disponible, sino usa Groq.
    """
...
def procesar_docx_multilingue(docx_path, docx_salida_path, groq_api_key, idioma_origen="inglés", tamano_lote=None):
    """Lee el DOCX original, traduce párrafos en lotes al español y guarda un nuevo DOCX limpio."""
...
# Mostrar advertencias si faltan dependencias opcionales
if not PYTESSERACT_AVAILABLE:
    st.info("ℹ️ pytesseract no instalado. La función OCR en imágenes estará deshabilitada. Para activarla, instala 'pytesseract' y Tesseract OCR.")
if not LANGDETECT_AVAILABLE:
    st.info("ℹ️ langdetect no instalado. La detección de idioma usará Groq (sigue funcionando, pero puedes instalar 'langdetect' para mayor precisión).")
        ...
        try:
            ...
            if exito_adobe:
                with st.spinner("🔍 Detectando idioma del documento..."):
                    ...
                    # Extraer muestra representativa (saltando portada/índice)
