...
# =============================================================================
# 4. NUEVA FUNCIÓN: CONVERTIR JSON (ADOBE EXTRACT) A WORD
# =============================================================================

def group_elements_by_line(elements: List[Dict], page_num: int) -> List[List[Dict]]:
    """Agrupa elementos de una misma página por línea (mismo valor Y dentro de una tolerancia)."""
    # Filtrar elementos de la página actual
    page_elements = [e for e in elements if e.get('Page') == page_num and 'Text' in e and e.get('Text', '').strip()]
    ...
    current_line = []
    current_y = None
    # Tolerancia de 5 píxeles (puede ajustarse)
    y_tolerance = 5
    ...
    for elem in page_elements:
        ...
        if current_y is None or abs(y - current_y) <= y_tolerance:
            ...
            # Ordenar los elementos de la línea por X (left)
...
def convert_json_to_word(json_bytes: bytes, config: CleanupConfig) -> bytes:
    """Convierte un JSON de Adobe Extract (structuredData.json) a un documento Word."""
    data = json.loads(json_bytes)
    elements = data.get('elements', [])
    
    # Obtener número de páginas
    pages = sorted(set(e.get('Page', 1) for e in elements if 'Page' in e))
    if not pages:
        pages = [1]
    ...
    # Para cada página, generar párrafos en orden de lectura
    for page_num in pages:
        # Agrupar elementos en líneas
        lines = group_elements_by_line(elements, page_num)
        
        for line in lines:
            # Concatenar los textos de la línea con espacios
            line_text = ' '.join(elem.get('Text', '') for elem in line).strip()
            if line_text:
                # Intentar detectar si es un título (ejemplo: texto corto en su propia línea, sin punto final)
                is_heading = (len(line_text) < 100 and not line_text.endswith('.')) or len(line) == 1
                para = doc.add_paragraph()
                run = para.add_run(line_text)
                if is_heading:
                    run.bold = True
                # No añadir sangría ni espaciado extra
                para.paragraph_format.space_after = Inches(0.05)
        
        # Opcional: agregar un salto de página entre páginas (excepto después de la última)
        if page_num != pages[-1]:
            doc.add_page_break()
...
# --- Pestaña o sección para PDF/Imagen ---
...
# --- Nueva opción: Convertir JSON a Word ---
st.markdown("---")
